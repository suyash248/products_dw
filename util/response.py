    ...
    def _intercept_decorator(func):
        def wrapper(controller, *args, **kwargs):
            try:
                ...
                # Default exceptions thrown by psycopg2 are handled here.
                import traceback
                print(traceback.format_exc())

                message = 'Database error occurred - {}'.format(dbe.diag.message_primary)
                response_obj = Response(success=False, status_code=500, message=message,
                                        errors=[Error(ErrorCode.DB_ERROR, message=str(dbe)).to_dict])
